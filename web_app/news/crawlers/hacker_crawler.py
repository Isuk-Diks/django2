...
def crawl_one(url):
    ...
    date = response.html.xpath(
        "/html/body/main/div/div[1]/div/div/div/div/div/div/div[4]/div/span[1]"
    )[0].text
    slug = slugify(title)
    ...
    article, created = Article.objects.update_or_create(
        source=url,
        defaults={"slug": slug, "title": title, "text": text, "created": pub_date},
    )
...
def crawl_urls():
    ...
    urls = response.html.xpath('//a[@class="story-link"]/@href')
    return urls
